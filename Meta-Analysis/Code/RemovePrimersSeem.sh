...
# Define directories
export DEMUX_DIR=${WORK_DIR}/bbmap_seem/demux
export P515F_DIR=${WORK_DIR}/bbmap_seem/515F
export P806R_DIR=${WORK_DIR}/bbmap_seem/806R
export PAIRED_DIR=${WORK_DIR}/bbmap_seem/paired       # Directory for paired reads
export MAPPING_FILE_PATH=${WORK_DIR}/bbmap_seem/mapping_file_seem.txt  # Ensure correct mapping file path

# Create necessary directories if they don't exist
mkdir -p "${LOGS_DIR}" "${DEMUX_DIR}" "${P515F_DIR}" "${P806R_DIR}" "${PAIRED_DIR}" "${FILTERED_DIR}"
...
while IFS=$'\t' read -r SAMPLEID MATCHID; do
    ...
    bbduk.sh \
            in="${DEMUX_DIR}/${SAMPLEID}"_1.fastq.gz \
            restrictleft=30 \
            k=19 \
            hdist=1 \
    ...
    bbduk.sh \
            in="${DEMUX_DIR}/${SAMPLEID}"_2.fastq.gz \
            restrictleft=30 \
            k=19 \
            hdist=1 \
            literal=GTGYCAGCMGCCGCGGTAA \
...
find bbmap_seem  -type f -name "*.fastq" -exec gzip {} +
