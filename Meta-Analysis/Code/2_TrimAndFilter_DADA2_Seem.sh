...
WORK_DIR=$( pwd )
export WORK_DIR
export LOGS_DIR=${WORK_DIR}/bbmap_seem/logs
...
export PAIRED_DIR=${WORK_DIR}/bbmap_seem/paired       # This is not yet customizable; do not change
export FILTERED_DIR=${WORK_DIR}/bbmap_seem/filtered   # This is not yet customizable (or used); do not change
export MAPPING_FILE_PATH=${WORK_DIR}/bbmap_seem/mapping_file_seem.txt
...
for TASK_ID in $(seq 1 ${JOB_COUNT}); do
    ...
    ./2_trim_and_filter_DADA2_seem.R \
        -s "${SAMPLEID}" \
