...
save.image(file = file.path(getwd(), "bbmap_seem/dada2_seem.Rdata"))
...
seqtab.agg_seem <- seqtab.agg[rownames(seqtab.agg) != "empty", colnames(seqtab.agg) != "empty"]
#table(nchar(colnames(seqtab)))

# Set seqtab storage location
dir.create(paste(getwd(), "bbmap_seem/seqtabs", sep = "/"))

# Save complete R session in case user would like to inspect intermediate outputs
save.image(file = file.path(getwd(), "bbmap_seem/seqtabs/dada2_seem.Rdata"))

# Save ASV table to serialized file
saveRDS(seqtab.agg_seem, file = paste(getwd(),"bbmap_seem/seqtabs/seqtab.agg_seem.RDS", sep = "/"))
